...
def encrypt_message(
    ...
    return [
        pow(block, e, n) for block in get_blocks_from_text(message, block_size)
    ]
...
def encrypt_and_write_to_file(
    ...
    if key_size < block_size * 8:
        sys.exit(
            f"ERROR: Block size is {block_size * 8} bits and key size is {key_size} bits. The RSA cipher requires the block size to be equal to or greater than the key size. Either decrease the block size or use different keys."
        )
...
def main() -> None:
    ...
    if mode == "decrypt":
        ...
        encrypted_text = encrypt_and_write_to_file(filename, "rsa_pubkey.txt", message)
